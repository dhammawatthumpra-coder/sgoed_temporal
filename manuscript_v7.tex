...
coupled? Using a disciplined audit (reproduce, significance with
\(\ge 10\) seeds, null test, labeling test, thermalization check,
mechanism), we demonstrate that it cannot: \textbf{eleven structural
observables} (asymmetry ratio, sign counts, Myrheim--Meyer dimension,
spectral dimension, cycle ratio, time-reversal, order statistics,
eigenvalue repulsion, entropy rate, \ldots) are all time-symmetric or
...
\subsection{1 The matrix unit (v7
...
first-order bistability/hysteresis. This mechanism is the reproducible
core of the project; it picks \emph{a} direction but does not create
\emph{flow}.

\subsection{2 Growth, transport, and
...
\subsection{3 Observables}\label{observables}
...
\subsection{3 Route 2: Sequential growth and its
...
\subsection{4 Route 3: Non-equilibrium upwind
...
\subsection{5 The 4D question: matrix spontaneous symmetry
...
\cite{kim_nishimura_2019}, \cite{ishibashi_kawai_kitazawa_tsuchiya_1997}{]}, we asked whether the
...
\item
  ...
  \cite{kim_nishimura_2019}{]}; a CPU toy without it has no controlled
  configuration space.
\item
...
\subsection{6 Causal-set dimensions: percolation negative,
light-cone
...
\subsection{7 Spectral dimension on flow
...
programs {[}\cite{kim_nishimura_2011}, \cite{kim_nishimura_2019}{]}).
...
\item
  ...
  results) and \texttt{notes/SGOED\_v8..v14\_notes.md}.
\end{itemize}
...
(updates 1--12) in \texttt{matrix/SGOED\_TIME\_EMERGENCE\_SUMMARY.md};
project context in \texttt{SGOED\_PROJECT\_SUMMARY.md}.

\section{Trap Catalog}\label{appendix-b-trap-catalog}

Every metric that ``looked exciting'' in this project failed exactly one
of the gates below. The catalog is supplied as a structured table plus a
runnable toolbox (\texttt{code/audit\_gates.py}, Supplement A); the full
12-trap catalog and an eight-step auditing workflow ship as Supplement B
