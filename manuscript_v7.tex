...
\item
  ...
  (prefix-sum coupling).
\item
  \textbf{Non-equilibrium upwind transport} --- a directed source$\rightarrow$sink
...
philosophy \cite{page_wootters_1983}, \cite{barbour_1999},
\cite{rovelli_2018}, thermodynamics, and relational physics
\cite{connes_rovelli_1994}, \cite{rovelli_rqm_1996}. Our working hypothesis,
following the Structural Time Framework \cite{chanpengpad_stf} and
its SGOED implementation \cite{chanpengpad_sgoed_v2}, is relational: time
...
observer system \cite{chanpengpad_sgoed_v2}, graphs, hypergraphs,
...
\section{Models and Observables}\label{models-and-observables}

\subsection{The matrix unit (v7
...
IKKT-like family \cite{ishibashi_kawai_kitazawa_tsuchiya_1997} with stability gates) is
...
\(\sim 60\sigma\) over baseline \cite{chanpengpad_sgoed_v2}), with
first-order bistability/hysteresis (audited scripts
\texttt{AUDIT\_v7\_hysteresis*} at the repo root; see also the v6
preprint). This mechanism is the reproducible core of the project; it
picks \emph{a} direction but does not create \emph{flow}.

\subsection{Growth, transport, and
...
\tightlist
\item
  \textbf{Sequential growth (CSG-style \cite{rideout_sorkin_2000}):}
...
\item
  \textbf{Causal sets / posets} \cite{sorkin_2005}: relations
...
box effects; estimator after Myrheim \cite{myrhem_1978});
...
\section{The Six-Step Audit}\label{the-six-step-audit}
...
earlier branch \cite{chanpengpad_sgoed_v2}) \\
3. Null test & the effect separates from shuffle/baseline ($\gtrsim$ \(3\sigma\)
...
--- Section
\hyperref[route-2-sequential-growth-and-its-mechanism]{4.3},
\hyperref[route-1-past-hypothesis]{4.2}) \\
...
\subsection{Equilibrium is
time-symmetric}\label{equilibrium-is-time-symmetric}

In equilibrium Metropolis sampling \cite{metropolis_1953}, no
...
\subsection{Route 2: Sequential growth and its
...
\subsection{Route 3: Non-equilibrium upwind
...
\endlastfoot
\textbf{REAL (pump@0)} &
\([28.8,\allowbreak\,1.72,\allowbreak\,0.117,\allowbreak\,0.0247,\allowbreak\,0.020,\allowbreak\,0.005]\) &
\textbf{\(+5.773\pm0.016\)} \\
...
\subsection{The 4D question: matrix spontaneous symmetry
breaking}\label{the-4d-question-matrix-spontaneous-symmetry-breaking}

Motivated by the Lorentzian IKKT programs \cite{kim_nishimura_2011},
\cite{kim_nishimura_2012}, \cite{ishibashi_kawai_kitazawa_tsuchiya_1997}, we asked whether the
...
\item
  ...
  determinant as regulator \cite{kim_nishimura_2011},
  \cite{kim_nishimura_2012}; a CPU toy without it has no controlled
  configuration space.
\item
...
dimensionally reduce \cite{ishibashi_kawai_kitazawa_tsuchiya_1997}.

\subsection{Causal-set dimensions: percolation negative,
light-cone
...
\subsection{Spectral dimension on flow
...
\cite{ambjorn_jurkiewicz_loll_2005}, which evaluate \(d_s\) on equilibrium
...
earlier v5$\rightarrow$v6 correction \cite{chanpengpad_sgoed_v2}).
...
programs \cite{kim_nishimura_2011}, \cite{kim_nishimura_2012}).
...
discovery (light-cone locality $\rightarrow$ scale-invariant causal dimension) and
...
\appendix
\section{Reproducibility}\label{appendix-a-reproducibility}\begingroup\small\sloppy
...
\item
  Past hypothesis: \texttt{matrix/step\_past\_hypothesis.py} (\(N=4\), 3
  seeds).
\item
  Sequential growth: \texttt{matrix/step\_sequential\_growth.py}
  (\(M=8..32\), \(g_{\mathrm{inter}}=20\), \(n_{\mathrm{therm}}=100\));
  mechanism: \texttt{matrix/step\_growth\_mechanism.py}; past-hypothesis
  variant: \texttt{matrix/step\_growth\_past\_hypothesis.py}
  (\(n_{\mathrm{therm}}=120\)).
\item
  Transport: \texttt{matrix/step\_langevin\_transport\_tuned.py} +
  \texttt{matrix/step\_transport\_robust.py} (8 seeds \(\times\) 500
  steps; scan \(g_{\mathrm{trans}}\times g_{\mathrm{sink}}\)).
\item
  4D probes: \texttt{matrix/sgoed\_matrix\_v15.py},
  \texttt{matrix/step\_v15\_dynamical.py},
  \texttt{matrix/step\_v15\_bounded.py}.
\item
  Causal sets: \texttt{matrix/step\_causal\_set\_dmm.py},
  \texttt{matrix/step\_causal\_set\_scale\_study.py} (bitset transitive
  closure; per-\(N\) calibration),
  \texttt{matrix/step\_growth\_lightcone.py},
  \texttt{matrix/step\_lightcone\_followup.py}.
\item
  Spectral dimension: \texttt{matrix/step\_spectral\_dimension\_flow.py}
  (lazy walk).
\item
  ...
  B ships in \texttt{matrix/} (v8--v14 cores, audit scripts, results)
  and the per-version notes in \texttt{matrix/}. The
  bistability/hysteresis claim of Section 2.1 is backed by
  \texttt{AUDIT\_v7\_hysteresis.py} +
  \texttt{AUDIT\_v7\_hysteresis\_results.json} (repo root).
\item
  \textbf{Self-tuning variants audited:} degree-feedback (SOC-style,
  \texttt{matrix/step\_growth\_soc.py}) fails the scale-invariance gate
  outright in all 7 configs (std across \(N\) of 0.25--1.09);
  curvature-feedback (\texttt{c\_eff},
  \texttt{matrix/step\_growth\_ceff.py}) fails outright in 2 of 12
  configs (std 0.45--0.48), while its 9 remaining non-control runs sit
  pinned at the calibration ceiling (\(d=8.00\), std \(=0\) --- a
  boundary artifact, 5 of them yielding NaN \(c_{\mathrm{eff}}\)
  statistics --- not genuine invariance); only the static-cone control
  passes genuinely (std 0.006). Per-config results:
  \texttt{matrix/step\_growth\_soc\_results.json},
  \texttt{matrix/step\_growth\_ceff\_results.json}; full record in the
  living summary (updates 13--14).
\end{itemize}

Seeds: \(\{42,\dots,53\}\) ($\geq$10 for fine grids; $\geq$6 elsewhere, as
reported). Detailed reproducibility notes and the full results chain
(updates 1--17) in \texttt{matrix/SGOED\_TIME\_EMERGENCE\_SUMMARY.md};
project context in \texttt{SGOED\_PROJECT\_SUMMARY.md}.

\endgroup\section{Trap Catalog}\label{appendix-b-trap-catalog}
...
results backing each row ship in \texttt{matrix/}.
