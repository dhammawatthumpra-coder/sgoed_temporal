...
Supplement A) with the full v8--v14 evidence slice
(`audit_evidence/`).
...
bistability/hysteresis (audited scripts `AUDIT_v7_hysteresis*` in
`audit_evidence/`; see also the v6 preprint). This mechanism is the
reproducible core of the project; it picks *a* direction but does not
create *flow*.
...
- Past hypothesis: `code/step_past_hypothesis.py` ($N=4$, 3 seeds).

- Sequential growth: `code/step_sequential_growth.py` ($M=8..32$,
  $g_{\mathrm{inter}}=20$, $n_{\mathrm{therm}}=100$); mechanism:
  `code/step_growth_mechanism.py`; past-hypothesis variant:
  `code/step_growth_past_hypothesis.py` ($n_{\mathrm{therm}}=120$).

- Transport: `code/step_langevin_transport_tuned.py` +
  `code/step_transport_robust.py` (8 seeds $\times$ 500 steps; scan
  $g_{\mathrm{trans}}\times g_{\mathrm{sink}}$).

- 4D probes: `code/sgoed_matrix_v15.py`, `code/step_v15_dynamical.py`,
  `code/step_v15_bounded.py`.

- Causal sets: `code/step_causal_set_dmm.py`,
  `code/step_causal_set_scale_study.py` (bitset transitive closure;
  per-$N$ calibration), `code/step_growth_lightcone.py`,
  `code/step_lightcone_followup.py`.

- Spectral dimension: `code/step_spectral_dimension_flow.py` (lazy
  walk).
...
- Audit toolbox (Supplement A): `code/audit_gates.py` --- six gates as
  ...
  ships in `audit_evidence/` (v8--v14 cores, audit scripts, results) and
  the per-version notes in `notes/`. The bistability/hysteresis claim of
  Section 2.1 is backed by
  `audit_evidence/code/AUDIT_v7_hysteresis.py` + its result JSON.

- **Self-tuning variants audited:** degree-feedback (SOC-style,
  `code/step_growth_soc.py`) fails the scale-invariance gate outright in
  all 7 configs (std across $N$ of 0.25--1.09); curvature-feedback
  (`c_eff`, `code/step_growth_ceff.py`) fails outright in 2 of 12
  ...
  `results/step_growth_soc_results.json`,
  `results/step_growth_ceff_results.json`; full record in the living
...
`notes/SGOED_TIME_EMERGENCE_SUMMARY.md`; project context in
`notes/SGOED_PROJECT_SUMMARY.md`.
...
scripts and results backing each row ship in `audit_evidence/`.
