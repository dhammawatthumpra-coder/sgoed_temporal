...
- **Self-tuning variants audited (negative, §4.5–4.6):** degree-feedback (SOC-style, `code/step_growth_soc.py`) and curvature-feedback (`c_eff`, `code/step_growth_ceff.py`) growth laws fail the scale-invariance gate (std across $N$ of 0.25–1.09 vs the $\le0.05$ pass bound); per-config results in `code/step_growth_soc_results.json` and `code/step_growth_ceff_results.json`, full record in the living summary (updates 13–14).
