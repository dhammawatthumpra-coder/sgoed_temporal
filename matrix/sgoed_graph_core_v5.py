"""
SGOED-Relational Core v5 — Observer-Extent Gate (ceiling on obs_extent)
=======================================================================
Extends v4. The v4 hysteresis revealed that obs_extent (the observer internal
clique density) can run away to hundreds/thousands because the existing gate
only caps node DEGREE, not the observer's internal extent.

v5 adds an Observer-Extent Gate, mirroring the v7 matrix model's extent gate:
    if obs_extent > extent_max_obs:
        S += lambda_extent * (obs_extent - extent_max_obs)^2

where obs_extent = mean_{a,b in S, a!=b} W[a,b]^2  (mean internal edge weight^2).

The delta is O(d^2): obs_extent changes only through the (i,j) term when both
i and j are observer nodes, so the gate delta is a single-term update.

Defaults: extent_max_obs = inf (gate off, reproduces v3/v4); lambda_extent=10.0.
"""
import numpy as np
...
def compute_full_action_v5(
    ...
    # Observer-extent gate (new in v5)
    s_ext = 0.0
    obs_extent = compute_obs_extent(W, d)
    if obs_extent > extent_max_obs:
        s_ext += lambda_extent * ((obs_extent - extent_max_obs) ** 2)

    return s_sparsity + s_trans + s_gate + s_coup + s_fb + s_ext


@njit(fastmath=True)
def compute_delta_edge_v5(
    W, W2, out_deg, in_deg,
    i, j, new_val,
    d, alpha, beta, lambda_gate, k_max,
    g_xy, g_yx,
    ...
    # Observer-extent gate delta (O(d^2)); only internal edges matter
    delta_extent = 0.0
...
def run_v5(N=16, d=3, g_xy=0.8, g_yx=0.0,
    ...
    """v5 entry point. Adds an observer-extent gate on top of v4's anneal
    support. extent_max_obs=np.inf disables the gate (reproduces v4)."""
    if W_init is None:
        W_init = np.zeros((N, N))
        use_init = 0
    else:
